...
@OpenAPIDefinition(info = @Info(title = "Salve", version = "1.0", description = "API do salve food, ta com fome manda um salve", contact = @Contact(name = "Bernardo", email = "bernardogomesrib@gmail.com", url = "https://naotenhosite.com/"), license = @License(name = "Licence name", url = "https://some-url.com"), termsOfService = "Terms of service"), servers = {
    @Server(url = "http://${local.ip}:8080", description = "Remote server"),
...
@SecurityScheme(name = "bearerAuth", description = "JWT auth description", scheme = "bearer", type = SecuritySchemeType.OAUTH2, flows = @OAuthFlows(implicit = @OAuthFlow(authorizationUrl = "http://${local.ip}:9080/realms/salve/protocol/openid-connect/auth", tokenUrl = "http://${local.ip}:9080/realms/salve/protocol/openid-connect/token")), bearerFormat = "JWT", in = SecuritySchemeIn.HEADER)
